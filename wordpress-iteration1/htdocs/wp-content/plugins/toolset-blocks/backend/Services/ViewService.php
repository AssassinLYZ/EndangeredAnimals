...
class ViewService {
	...
	protected function render_preview_html( $id, $view_data ) {
		...
		do_blocks( $view_data['general']['view_template'] );
	...
	public function save( $id, $view_data ) {
		...
		$data = $parser->find_block_in_text( $view_template, 'toolset-views/custom-search-container' );
		if ($data != null) {
			$view_data['loop']['filter_template'] = substr($view_template, $data['start'], $data['end'] - $data['start']);
		...
		$view_layout = '[wpv-layout-start]' .
						substr($view_layout, 0, $data['start']) .
					   $view_data['loop']['loop_template'] .
					   substr($view_layout, $data['end']) .
		               '[wpv-layout-end]';
