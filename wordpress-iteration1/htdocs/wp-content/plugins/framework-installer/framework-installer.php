...
function fidemo_get_reference_sites() {
	...
	if ( $diff > 7200 || empty( $sites ) || ( isset( $_GET['force_refresh'] ) && $_GET['force_refresh'] == 1 ) ) {
		...
		if ( empty( $ref_sites ) ) {
			global $frameworkinstaller;
			add_action( 'admin_notices', array( $frameworkinstaller, 'connection_refused_notice' ) );
...
function fidemo_check_themes_availability( $sites ) {
	...
	for ( $i = 0; $i < count( $sites ); $i ++ ) {
		...
		if ( ! isset( $themes[ $sites[ $i ]->themes->theme ] )
			|| version_compare( $sites[ $i ]->themes->theme_version, $themes[ $sites[ $i ]->themes->theme ] ) !== 0 ) {
		...
		if ( isset( $sites[ $i ]->themes->additional_themes ) ) {
			foreach ( $sites[ $i ]->themes->additional_themes as $theme => $theme_object ) {
				if ( ! isset( $themes[ $theme ] )
					|| version_compare( $theme_object->version, $themes[ $theme ] )
					!== 0 ) {
	...
	foreach ( $sites as $index => $site ) {
		if ( intval( $site->ID ) == intval( $site_id ) ) {
			...
			foreach ( $plugins as $plugin => $plugin_info ) {
				...
				if ( ( strpos( $plugin_name, 'WPML' ) !== false
					|| strpos( $plugin_name, 'Multilingual' )
					!== false ) ) {
					return true;
				}
			}
...
/**
 * Init Installer class
 */
function fidemo_installer_init() {
	$installer = FIDEMO_ABSPATH . '/vendor/otgs/installer/loader.php';
	if ( file_exists( $installer ) ) {

		require_once FIDEMO_ABSPATH . '/vendor/jakeasmith/http_build_url/src/http_build_url.php';
